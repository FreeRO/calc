...
function BattleCalc999() {
    ...
    if (str_bSUBname = "", str_bSUB = "", 0 == n_A_ActiveSkill || 272 == n_A_ActiveSkill || 401 == n_A_ActiveSkill || 86 == n_A_ActiveSkill && 50 <= n_B[3] && n_B[3] < 60 || (myInnerHtml("CRIATK", "", 0), myInnerHtml("CRInum", "", 0), myInnerHtml("CRIATKname", "", 0), myInnerHtml("CRInumname", "", 0)), 10 != n_A_WeaponType && 17 != n_A_WeaponType && 18 != n_A_WeaponType && 19 != n_A_WeaponType && 20 != n_A_WeaponType && 21 != n_A_WeaponType || 0 != n_A_ActiveSkill || (n_Enekyori = 1), 0 == n_A_ActiveSkill || 86 == n_A_ActiveSkill && 50 <= n_B[3] && n_B[3] < 60)
        ...
        if (n_A_ActiveSkill == w_ActS[iw]) {
            ...
            ATKbai02(wbairitu, 0);
            for (var i = 0; 2 >= i; i++) w_DMG[i] = BattleCalc(n_A_DMG[i], i), 391 == n_A_ActiveSkill && 2 != n_B[2] && 4 != n_B[2] && (w_DMG[i] = 0), w_DMG[i] += EDP_DMG(i), Last_DMG_B[i] = w_DMG[i], 76 == n_A_ActiveSkill && (Last_DMG_B[i] = 2 * w_DMG[i]), Last_DMG_A[i] = w_DMG[i] * wHITsuu, 0 == n_B_IJYOU[6] || 0 == wLAch ? InnStr[i] += Math.floor(w_DMG[i] * wHITsuu) + " (" + w_DMG[i] + SubName[8] + wHITsuu + "hit)" : (InnStr[i] += 3 * w_DMG[i] + "(" + 2 * w_DMG[i] + "+" + w_DMG[i] + ")", Last_DMG_B[i] = 3 * w_DMG[i]), w_DMG[i] -= EDP_DMG(i), w_DMG[i] *= wHITsuu;
            var wX = BattleCalc2(0);
            ...
            if (n_PerHIT_DMG = 0, n_Enekyori = 1, n_A_Weapon_zokusei = 0, n_Delay[2] = .7, wbairitu2 = 1 + .3 * n_A_ActiveSkillLV, 384 == n_A_ActiveSkill && (n_Delay[2] = .35, wbairitu2 *= 2), SRV) {
                ...
                for (var i = 0; 2 >= i; i++) n_tok[23] > 0 ? (n_A_ATK_IP = Math.round((SB_ATK + ItemOBJ[n_A_Equip[5]][6]) * (n_B_DEF2[2 - i] + n_B[14]) / 100), w_DMG[i] = n_A_ATK_IP * wbairitu) : (w_DMG[i] = (SB_ATK + ItemOBJ[n_A_Equip[5]][6]) * wbairitu, w_DMG[i] = Math.floor(w_DMG[i] * (100 - (n_B[14] - M_DEF1)) / 100 - (n_B_DEF2[i] - M_DEF2))), w_DMG[i] = Math.floor(w_DMG[i] * wbairitu2), w_DMG[i] = BaiCI(w_DMG[i]) + wSBr, 0 != M_DEF1 && (w_DMG[2] = w_DMG[1] = w_DMG[0]), w_DMG[i] < 1 && (w_DMG[i] = 1), 305 == ItemOBJ[n_A_Equip[5]][0] && (w_DMG[i] = 0), w_DMG[i] = Math.floor(w_DMG[i] * zokusei[n_B[3]][0]), Last_DMG_A[i] = Last_DMG_B[i] = w_DMG[i], InnStr[i] += Last_DMG_A[i]
            } else {
...
function HealCalc(_, n) {
    ...
    var i = 100 + n_tok[91];
    return 1 == n && (i += n_tok[92]), EquipNumSearch(644) && (i += Math.floor(1.5 * n_A_Weapon_ATKplus)), EquipNumSearch(565) && n_A_HEAD_DEF_PLUS >= 7 && (i += 1 * EquipNumSearch(565)), EquipNumSearch(1277) && n_A_HEAD_DEF_PLUS >= 7 && (i += 3 * EquipNumSearch(1277)), wHeal = Math.floor(wHeal * (i / 100) * (1 - 20 * n_A_Buf6[18] / 100)), wHeal
}
...
function BattleHiDam() {
    ...
    else if (484 == n_B_AtkSkill) n_B_Weapon_zokusei = document.calcForm.B_RandomElementAtkSkill.selectedIndex, atkmod += 100 * n_B_AtkSkillLV - 100;
    ...
    else if (128 == n_B_AtkSkill) n_B_rangedMAtk = 1, n_B_Weapon_zokusei = 1, BskillHitNum = n_B_AtkSkillLV >= 4 ? 25 : n_B_AtkSkillLV >= 2 ? 9 : 1, atkmod += 30 * n_B_AtkSkillLV;
    ...
    for (wBHD = 0, i = 0; 6 >= i; i++) wBHD += w_HiDam[i];
...
function ClickJob(n) {
    with(document.calcForm) {
        ...
        if (n_SkillSW && (45 == n_A_JOB ? (myInnerHtml("AS12_1", "-", 0), A2_Skill12.style.visibility = "hidden", A2_Skill12.value = 0, n_A_Buf2[12] = 0) : (myInnerHtml("AS12_1", "# of Spirit Spheres", 0), A2_Skill12.style.visibility = "visible"), 13 == n_A_JOB || 27 == n_A_JOB ? (myInnerHtml("AS11_1", "-", 0), A2_Skill11.style.visibility = "hidden", A2_Skill11.value = 0, n_A_Buf2[11] = 0) : (myInnerHtml("AS11_1", "Providence", 0), A2_Skill11.style.visibility = "visible")), n_Skill7SW) {
            for (var i = 2; 3 >= i; i++) A_SpeedPOT.options[2] = null;
            3 != n_A_JOB && 9 != n_A_JobClass2() && 16 != n_A_JobClass2() ? A_SpeedPOT.options[2] = new Option(SpeedPotName[2] + " (Lv40)", 2) : A_SpeedPOT.options[2] = new Option("-", 0), 1 == n_A_JobClass() || 6 == n_A_JobClass() || 41 == n_A_JobClass() || 14 == n_A_JobClass2() || 11 == n_A_JobClass2() || 5 == n_A_JOB || 45 == n_A_JOB ? A_SpeedPOT.options[3] = new Option(SpeedPotName[3] + " (Lv85)", 3) : 22 == n_A_JOB ? A_SpeedPOT.options[3] = new Option("* Special (" + SkillOBJ[304][2] + " Lv85) / Poison Bottle", 3) : A_SpeedPOT.options[3] = new Option("* Special (" + SkillOBJ[304][2] + ") (Lv85)", 3)
        }
        for (20 != n_A_JOB && (SuperNoviceFullWeaponCHECK = 0), SuperNoviceFullWeaponCHECK ? JobASPD[20][7] = 120 : JobASPD[20][7] = 0, 0 == SRV && 8 == n_A_JobClass2() && 1 == n_Nitou && (n_Nitou = 0, ClickWeaponType2(0)),
            i = 21; i >= 0; i--) A_WeaponType.options[i] = null;
...
function ClickWeaponType(n) {
    with(document.calcForm) {
        ...
        } else A_Arrow.value = 0, A_Arrow.style.visibility = "hidden";
        WeaponSet(), 0 == n ? (A_Weapon_ATKplus.disabled = !0, A_Weapon_ATKplus.value = 0, 0 == n_Nitou && (A_LEFT_DEF_PLUS.disabled = !1, A_left.disabled = !1, A_left_card.disabled = !1)) : (A_Weapon_ATKplus.disabled = !1, 3 == n || 5 == n || 7 == n || 10 == n || 11 == n || 16 == n || 17 == n || 18 == n || 19 == n || 20 == n || 21 == n ? (A_LEFT_DEF_PLUS.disabled = !0, A_LEFT_DEF_PLUS.value = 0, A_left.disabled = !0, A_left.value = 305, A_left_card.disabled = !0, A_left_card.value = 0) : 0 == n_Nitou && (A_LEFT_DEF_PLUS.disabled = !1, A_left.disabled = !1, A_left_card.disabled = !1)), n_A_JobSet(), 8 != n_A_JOB && 22 != n_A_JOB || 11 == n ? (myInnerHtml("A_SobWeaponName", "", 0), myInnerHtml("spanA_weapon2", "", 0), myInnerHtml("spanA_weapon2seiren", "", 0), myInnerHtml("spanA_weapon2_CardShort", "", 0), myInnerHtml("nA_weapon2_c1", "", 0), myInnerHtml("nA_weapon2_c2", "", 0), myInnerHtml("nA_weapon2_c3", "", 0), myInnerHtml("nA_weapon2_c4", "", 0), n_Nitou = 0, 3 == n || 5 == n || 7 == n || 10 == n || 11 == n || 16 == n || 17 == n || 18 == n || 19 == n || 20 == n || 21 == n ? (A_LEFT_DEF_PLUS.disabled = !0, A_LEFT_DEF_PLUS.value = 0, A_left.disabled = !0, A_left.value = 305, A_left_card.disabled = !0, A_left_card.value = 0) : 0 == n_Nitou && (A_LEFT_DEF_PLUS.disabled = !1, A_left.disabled = !1, A_left_card.disabled = !1)) : 0 == n_Nitou && myInnerHtml("A_SobWeaponName", 'Left Hand: <select name="A_Weapon2Type" onChange = "ClickWeaponType2(this[this.selectedIndex].value) | StAllCalc()">	<option value="0">Fist or Shield<option value="1">Dagger<option value="2">Sword<option value="6">Axe</select>', 0), n_A_Equip[0] = eval(A_weapon1.value), ActiveSkillSetPlus(), ClickB_Item(n_A_Equip[0])
    }
}
...
function ClickWeaponType2(n) {
    with(document.calcForm) {
        if (n_A_JobSet(), 0 != n) {
            if (A_LEFT_DEF_PLUS.disabled = !0, A_LEFT_DEF_PLUS.value = 0, A_left.disabled = !0, A_left.value = 305, A_left_card.disabled = !0, A_left_card.value = 0, 0 == n_Nitou) {
                for (myInnerHtml("spanA_weapon2", '<select name="A_weapon2" onChange="StAllCalc()|ClickB_Item(this[this.selectedIndex].value)"></select>', 0), myInnerHtml("spanA_weapon2seiren", 'Refine (Left): <select name="A_Weapon2_ATKplus" onChange = "StAllCalc()"></select>', 0), i = 0; 10 >= i; i++) A_Weapon2_ATKplus.options[i] = new Option("+" + i, i);
...
function BufSW(v) {
    with(document.calcForm) {
        if (n_SkillSW = v, n_SkillSW) {
            ...
            for (45 == n_A_JOB ? (myInnerHtml("AS12_1", "-", 0), A2_Skill12.style.visibility = "hidden", A2_Skill12.value = 0, n_A_Buf2[12] = 0) : (myInnerHtml("AS12_1", "# of Spirit Spheres", 0), A2_Skill12.style.visibility = "visible"), 13 == n_A_JOB || 27 == n_A_JOB ? (myInnerHtml("AS11_1", "-", 0), A2_Skill11.style.visibility = "hidden", A2_Skill11.value = 0, n_A_Buf2[11] = 0) : (myInnerHtml("AS11_1", "Providence", 0), A2_Skill11.style.visibility = "visible"), i = 0; 3 >= i; i++) A2_Skill10.options[i] = new Option(i, i);
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="A2TD" class="subheader point" onclick="BufSW(1)">Supportive / Party Skills <span id="A2used"></span>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("SIENSKILL", str, 0)
        }
        A2(0)
    }
}
        ...
        if (0 != n_A_Buf3[3]) {
            if (0 == SWs3sw[3]) {
                for (0 == n_A_Buf3[23] && (n_A_Buf3[23] = 100,
                        n_A_Buf3[33] = 10), myInnerHtml("EN3_3", "Bard's VIT", 0), myInnerHtml("EN3_4", '<select name="A3_Skill3_2" onChange="A3(1)"></select>', 0), myInnerHtml("EN3_5", "Musical Lessons", 0), myInnerHtml("EN3_6", '<select name="A3_Skill3_3" onChange="A3(1)"></select>', 0), i = 1; 200 >= i; i++) A3_Skill3_2.options[i - 1] = new Option(i, i);
...
function Buf4SW(v) {
    with(document.calcForm) {
        if (n_Skill4SW = v, n_Skill4SW) {
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="A4TD" class="subheader point" onclick="Buf4SW(1)">Guild Skills <span id="A4used"></span>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("SP_SIEN02", str, 0)
        }
        A4(0)
    }
}
...
function Buf7SW(v) {
    with(document.calcForm) {
        if (n_Skill7SW = v, n_Skill7SW) {
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="A7TD" class="subheader point" onclick="Buf7SW(1)">Food / Speed Potions / other Items <span id="A7used"></span>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("SP_SIEN05", str, 0)
        }
        A7(0)
    }
}
...
function Buf8SW(v) {
    with(document.calcForm) {
        if (n_Skill8SW = v, n_Skill8SW) {
            var str;
            for (str = '<table class="tborder">', str += '<TR><TD id="A8TD" class="subheader point" onclick="Buf8SW(0)">Additional Effects <SPAN id="A8used"></SPAN>', str += '<div class="right">(click to show/hide)</div></TD></TR>', str += '<TR><TD><Font size="2"><B>Pets</B></Font></TD></TR>', str += '<TR><TD id="EN800" class="dotB"></TD></TR>', str += '<TR><TD id="EN809" class="dotB"></TD></TR>', str += '<TR><TD><Font size="2"><B>EXP Items & Variables</B></Font></TD></TR>', str += '<TR><TD id="EN803" class="data"></TD></TR>', str += '<TR><TD id="EN804" class="data"></TD></TR>', str += '<TR><TD id="EN805" class="data"></TD></TR>', str += '<TR><TD id="EN806" class="data"></TD></TR>', str += '<TR><TD id="EN801" class="data"></TD></TR>', str += '<TR><TD id="EN802" class="data"></TD></TR>', str += "</TABLE>", myInnerHtml("ID_ETC", str, 0), myInnerHtml("EN800", '<select name="A8_Skill0" onChange="A8(1)"></select>', 0), i = 0; i < PET_OBJ_SORT.length; i++) {
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="A8TD" class="subheader point" onclick="Buf8SW(1)">Additional Effects <SPAN id="A8used"></SPAN>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("ID_ETC", str, 0)
        }
        A8(0)
    }
}
...
function Buf9SW(v) {
    with(document.calcForm) {
        if (n_Skill9SW = v, n_Skill9SW) {
            ...
            for (i = 0; i < ZokuseiOBJ2.length; i++) document.calcForm.A9_Skill4.options[i] = new Option(ZokuseiOBJ2[i], i),
                document.calcForm.A9_Skill5.options[i] = new Option(ZokuseiOBJ2[i], i), document.calcForm.A9_Skill6.options[i] = new Option(ZokuseiOBJ2[i], i), document.calcForm.A9_Skill7.options[i] = new Option(ZokuseiOBJ2[i], i);
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="A9TD" class="subheader point" onclick="Buf9SW(1)">Additional Enchants & Manual Edits on Player <SPAN id="A9used"></SPAN>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("ID_ARG", str, 0)
        }
        A9(0)
    }
}
...
function Buf10SW(v) {
    with(document.calcForm) {
        if (n_Skill10SW = v, n_Skill10SW) {
            ...
            var str;
            str = '<table class="tborderA">', str += '<TR><TD id="A10TD" class="subheader point" onclick="Buf10SW(1)">Manual Edits on Enemy <SPAN id="A10used"></SPAN>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("B_MANUAL", str, 0)
        }
        A10(0)
    }
}
...
function IjyouSW(v) {
    with(document.calcForm) {
        if (n_IjyouSW = v, n_IjyouSW) {
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="AITD" class="subheader point" onClick="IjyouSW(1)">Debuffs on Enemy <span id="AIused"></span>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("MONSTER_IJYOU", str, 0)
        }
        AI(0)
    }
}
...
function EnemyKyoukaSW(v) {
    with(document.calcForm) {
        if (n_KyoukaSW = v, n_KyoukaSW) {
            ...
            var str;
            str = '<table class="tborder">', str += '<TR><TD id="AKTD" class="subheader point" onClick="EnemyKyoukaSW(1)">Buffs on Enemy <span id="AKused"></span>', str += '<div class="right">(click to show/hide)</div></TD></TR></TABLE>', myInnerHtml("MONSTER_KYOUKA", str, 0)
        }
        AK(0)
    }
}
...
function calc() {
    ...
    if (n_Max_DMG = 0, n_Min_DMG = 9999999, (0 == n_A_ActiveSkill || 86 == n_A_ActiveSkill && 50 <= n_B[3] && n_B[3] < 60) && w998G > 0 && (n_Min_DMG = n_A_CriATK[0], n_Max_DMG = n_A_CriATK[2]), BattleCalc999(), 0 == n_Enekyori) {
        var e = n_B_KYOUKA[12],
            i = n_B_manual[21];
        if (e > 0) {
            e = 10 + 3 * e;
...
], bBGC = ["#CDF", "#CCC", "#FDC", "#313", "#000"], hBGC1 = ["#0FB91E", "#57D", "#B44", "#622", "#444"], hBGC2 = ["#326B2D", "#24A", "#A33", "#411", "#222"], selBGC = ["#FC8", "#FC8", "#FC8", "#FC8", "#AAA"], ssBGC = ["#FFF", "#FFF", "#FFF", "#FC8", "#AAA"], sBGC = ["#466", "#36B", "#A33", "#626", "#000"], saBGC = ["#A52", "#811", "#3A3", "#A11", "#A11"], mBGC = ["#FFF", "#FFF", "#FFF", "#C8F", "#444"], tBGC = ["#FFF", "#FFF", "#FFF", "#C8F", "#555"];
var sheet = function() {
